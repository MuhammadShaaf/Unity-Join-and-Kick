    ...
    void Update()
    {
        if (gameState)
        {
            ...
            { 
                ...
                Direction.x = Mathf.Lerp(Direction.x,Input.GetAxis("Mouse X"), Time.deltaTime * runSpeed * 2 );
    ...
    private void FixedUpdate()
    {
        if (gameState)
        {
            if (MoveByTouch)
            {
                ...
                foreach (var stickman_rb in Rblst)
                    stickman_rb.velocity = new Vector3(Direction.x * Time.fixedDeltaTime * swipeSpeed,0.7f,0.7f) + displacement;
            }
            else
            {
                foreach (var stickman_rb in Rblst)
